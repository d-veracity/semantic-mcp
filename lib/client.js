...
export class SemanticClient {
  ...
  async rawRequest(method, path, body) {
    ...
    if (res.status === 403 && data?.requiredTier) {
      throw new SemanticApiError(`Plan required: ${data.message || 'this feature is not on the current plan'}`, {
        status: 403,
        detail: data,
        actionable:
          `This tool needs the '${data.requiredTier}' plan or above; the account is on '${data.currentTier || 'free'}'. ` +
          'Ask your human operator to upgrade at https://dveracity.com/pricing, then retry. Do not retry on your own.',
      });
    }
    throw new SemanticApiError(
  ...
  /** Approved regulatory compliance policies (paid plans). Drafts are never returned. */
  compliancePolicies({ framework, domain, includeRetired } = {}) {
    const qs = new URLSearchParams();
    if (framework) qs.set('framework', framework);
    if (domain) qs.set('domain', domain);
    if (includeRetired) qs.set('include_retired', 'true');
    const q = qs.toString();
    return this.request('GET', `/api/v1/compliance/policies${q ? `?${q}` : ''}`);
  }

  /** One approved compliance policy with its Rego, extracted rules, inputs and provenance. */
  compliancePolicy(id) {
    return this.request('GET', `/api/v1/compliance/policies/${encodeURIComponent(id)}`);
  }
