...
export function registerTools(server, client) {
  // Every tool gets a top-level `title` as well as the one inside `annotations`.
  // The Claude Connectors review criteria read "every tool must include a
  // title", and their portal groups tools by annotation and flags missing
  // titles; the SDK's tool() form only carries title inside annotations, so
  // set it on the RegisteredTool it returns. No private fields involved.
  const register = server.tool.bind(server);
  const tool = (name, ...rest) => {
    const registered = register(name, ...rest);
    if (registered && typeof registered.update === 'function') registered.update({ title: name });
    return registered;
  };
  ...
  tool(
    ...
    'Check a data payload against the canonical Open Footprint model and, when a sector is named, ' +
      ...
      '(required fields, primary key, declared types) and the constraints the model declares (identifier ' +
      'patterns, enums, date formats, lengths); with a sector, the guardrails. WHAT IT DOES NOT CHECK in ' +
      'this release: value ranges (the model declares none, so a negative quantity passes), unit ' +
      'coherence, temporal consistency (an inverted validity period passes), referential integrity ' +
      '(foreign keys are pattern-checked, never resolved against real records), factor provenance, or ' +
      'materiality. The response says so per call: `checked` lists the checks that ran and ' +
      '`checks_not_run` lists every catalogued check that did not, each with a reason (not_implemented, ' +
      'no_data_plane, no_range_declared, no_sector_supplied, no_policy_published, not_applicable, ' +
      'unevaluable). `schemaValid` is the structural verdict; `assuranceLevel` (schema-only | ' +
      'schema-and-value | schema-value-and-policy | full) is the depth earned. `valid` is deprecated ' +
      '(kept through 0.5.x, removed no earlier than 0.6.0) — it only ever covered the checks that ran. ' +
      'Every violation and warning carries severity (error | warning); unknown fields are warnings, not ' +
      'violations. Guardrails report four distinct outcomes, never conflated: passed, denied, ' +
      'not_applicable (the rules do not cover this record type), or unevaluable (the payload lacks the ' +
      'e-ledger inputs the rules read, e.g. co2e_kg — listed in policy.missingInputs). Never report a ' +
      'payload as compliant unless sector_policy is in `checked` and policy.outcome is passed.',
    {
