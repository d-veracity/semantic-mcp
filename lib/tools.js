...
export function registerTools(server, client) {
  ...
  tool(
    ...
    'Check a data payload against the canonical Open Footprint model and, when a sector is named, ' +
      ...
      'patterns, enums, date formats, lengths); temporal consistency (a validity period whose end ' +
      'precedes its start is rejected); enum membership (a foreign key into a vocabulary that publishes ' +
      'its members is checked against that member set, so a well-formed identifier for a referent that ' +
      'does not exist is caught); and, with a sector, the guardrails. WHAT IT DOES NOT CHECK in this ' +
      'release: value ranges (the model declares none, so a negative quantity passes), unit coherence, ' +
